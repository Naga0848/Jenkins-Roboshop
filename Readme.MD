...
## Steps to be performed when we want to deploy the roboshop on an EKS Cluster
...
## Jenkins(catalogue-deplpyment) steps
...
## SCANS
   ...
   static source code analysis --> sonarqube  (to see whether we are following all the coding standards)

   static application secuirty testing --> sonarqube, fortify scan, github (this checks if we are unnecessarily printing the passwords in logs i.e., basically security related)

   open source library scan --> nexus iq, github dependabot (this scans our dependencis which we are in our code from external resources)

   dynamic application secuirty testing --> attacks on running application.. fortify webinspect, veracode --> Limited freetrail  (this is to test on a running application)

   docker image scan --> vulnerabilities in the images

   ## SonarQube Installation
   
     use the Docker >> docker-eks-via-terraform (docker.sh) repo to create a instance with dokcer and loginto the 
     the same instance
   ...
   ![alt text](images/sonar-on-docker.png)   
     ...
     sudo docker run -d --name sonarqube -p 9000:9000 sonarqube:community   >> command to launch sonarqube via doc
   
     publicIP:9090  in the browser to see the sonarqube and continue with the remaining Jenkins steps

     The default credentials for a fresh SonarQube Community Edition instance (including when run via your Docker command) are:
      ...
      Now, install SonarQube Scanner plugin which will provide us an option to add a scanner

   ![alt text](images/sonarqube-installation-on-jenkins.png)   

      Now, our Jenkins should now where the SonarQube is located and it should have a password or the secret text to  talk to the SonarQube
            
      Generating the secret text inside our SonarQube

   ![alt text](images/secret-text-generation.png)   
   
      Now attaching the secret-text inside our Jenkins(Manage-Jenkins) and from there we should give sonar-auth like how we gave ssh-auth for jenkins-agent

      Make sure the name should be same as sonar-8.0.1 which we have given while adding the scanner inside the jenkins
      

   ![alt text](images/adding-secret-text-on-jenkins.png)

   Now we will add a file named sonar-project.properties in our catalogue repo which will contain the details like what to scan and what to exclude and the name of the project

   And write the stage in the catalogue Jenkinsfile and build the pipeline without deploying it in to the environment 
   
   Please select the mode(standard) as shown in the below images

   ![alt text](images/mode-selection.png)

   ![alt text](images/standard-mode.png)

   


## Quality Gate Creation in SonarQube and Understanding Quality Profiles

   We should create a quaility gate and make it as default and start adding the rules for tha over all code

  ![alt text](images/new-code.png)

  ![alt text](images/overall-code.png)

  Now, we have to include a stage named Sonar Quality Gates to check the code related issues. For this we have to andable webhooks in the SonarQube as shown below, and the url is jenkins publicIP (http://publicIP:8080/)

  ![alt text](images/jenkins-webhook.png)

  Once webhook is enabled, the sonarqube will send back the results to Jenkins and Jenkins will wait for Quality Gates and accordingy it will fail or pass the Pipeline...

  The Quality Gate works according to the rules in the Quality Profiles. Quality Gate decides whether it is a bug or not llok at the rules mentioned in the Quality Profiles
  ...
  Quality Profiles:-
   ...
   These profiles are described by the top level tech people and the archeticts in our company.

   In the below mentioned image, we can find the Quality Profiles mentioned for a Java based application

  ![alt text](images/java-quality-profile.png) 
...
We need to generate token in our GitHub and use it in the Jenkinsfile
